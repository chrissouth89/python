...
class TicTacToe:
    ...
    def winner(self, square, letter):
        ...
        if all([spot == letter for spot in column]):
            ...
            # check diagonal even squares (0, 2, 4, 6, 8)
            # only possible diagonal wins
        if square % 2 == 0:
...
def play(game, x_player, o_player, print_game=True):
    # return winner of the game(the letter) or None for a tie
    if print_game:
        game.print_board_nums()

    letter = 'X'  # starting letter
    # while game is incomplete, continue iterating
